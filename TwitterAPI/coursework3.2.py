...
class TweetExtractor(tk.Frame):
    def __init__(self, parent):
        ...
        self.myStreamListener = tweepy.StreamListener
        self.myStream = tweepy.Stream(auth=self.api.auth, listener=self.myStreamListener)

        # Initialize tweets and conversation queue
    ...
    def twitterStream(self):
        while True:
            if self.myStream.running and self.is_paused:
                ...
                if location is None:
                    tweets = self.myStream.filter(is_async=True, track=self.filter_list_internal, languages=[language])
                    for tweet in tweets:
    ...
    def processTweets(self):
        while True:
            try:
                tweet = self.tweet_queue.get(block=True)
